...
class TestCreationTables:
    def test_table_sources_existe(self, db):
        assert "sources" in inspect(db.engine).get_table_names()

    def test_table_documents_existe(self, db):
        assert "documents" in inspect(db.engine).get_table_names()

    def test_table_journaux_existe(self, db):
        assert "journaux" in inspect(db.engine).get_table_names()

    def test_colonnes_sources(self, db):
        colonnes = {c["name"] for c in inspect(db.engine).get_columns("sources")}
        attendues = {
    ...
    def test_colonnes_documents(self, db):
        colonnes = {c["name"] for c in inspect(db.engine).get_columns("documents")}
        attendues = {
    ...
    def test_colonnes_journaux(self, db):
        colonnes = {c["name"] for c in inspect(db.engine).get_columns("journaux")}
        attendues = {
...
class TestModeleSource:
    def test_creation_source(self, db):
        src = _nouvelle_source()
        db.session.add(src)
        db.session.commit()
        assert src.id is not None

    def test_login_chiffre_en_base(self, db):
        src = _nouvelle_source()
    ...
    def test_login_dechiffre_a_la_lecture(self, db):
        src = _nouvelle_source()
        src.login = "admin_sftp"
    ...
    def test_login_vide_reste_vide(self, db):
        src = _nouvelle_source()
        src.login = ""
        db.session.add(src)
        db.session.commit()
    ...
    def test_valeurs_par_defaut(self, db):
        src = _nouvelle_source()
        db.session.add(src)
        db.session.commit()
    ...
    def test_unicite_nom(self, db):
        db.session.add(_nouvelle_source(nom="Doublon"))
        db.session.commit()
        db.session.add(_nouvelle_source(nom="Doublon"))
...
class TestModeleDocument:
    def _source(self, db):
        src = _nouvelle_source(nom="Source-Doc-Test")
        db.session.add(src)
        db.session.flush()
        return src

    def test_creation_document(self, db):
        src = self._source(db)
        doc = Document(
            source_id=src.id,
    ...
    def test_statut_par_defaut_ok(self, db):
        src = self._source(db)
        doc = Document(
            source_id=src.id,
    ...
    def test_statut_critique(self, db):
        src = self._source(db)
        doc = Document(
            source_id=src.id,
...
class TestModeleJournal:
    def test_creation_journal_sans_source(self, db):
        entree = Journal(
    ...
    def test_details_json_serialisation(self, db):
        entree = Journal(
    ...
    def test_details_none_reste_none(self, db):
        entree = Journal(
    ...
    def test_created_at_automatique(self, db):
        entree = Journal(
