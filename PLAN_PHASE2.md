...
- [x] **B5. Synchronisation groupes AD** (~2h) ✓
  - Mapper groupes AD vers rôles applicatifs (LDAP_GROUP_MAPPING)
  - Sync périodique des appartenances (job quotidien à 4h)
  - Interface admin `/admin/ldap` pour test et sync manuel
...
- [x] **D2. Rapport Excel** (~1h) ✓
  - Installer `openpyxl`
  - Export XLSX avec feuilles par source + feuille récapitulative

- [x] **D3. Planification rapports** (~1h) ✓
  - Génération automatique quotidienne/hebdomadaire/mensuelle
  - Envoi par email aux destinataires configurés
  - Configuration via `/admin/parametres`
...
- [x] **E5. Endpoints monitoring** (~30min) ✓
  - `GET /api/v1/health` — état de l'application
  - `GET /api/v1/stats` — statistiques globales (nb docs, alertes, etc.)

- [x] **E6. Documentation OpenAPI** (~1h) ✓
  - Spécification OpenAPI 3.0 sur `/api/v1/openapi.json`
  - Swagger UI sur `/api/v1/docs`

---

## F — Améliorations diverses (hors CDC mais utiles)

- [x] **F1. Page paramètres globaux** (~1h) ✓
  - Interface web `/admin/parametres` : fuseau horaire, rapports auto, corbeille
  - Table `settings` clé/valeur avec modèle SQLAlchemy

- [x] **F2. Corbeille avec période de grâce** (~1h) ✓
  - Documents déplacés vers `_corbeille/` (déjà implémenté Phase 1)
  - Nettoyage automatique après X jours (configurable)
  - Job de nettoyage quotidien à 3h

- [x] **F3. Recherche texte** (~30min) ✓
  - Recherche par nom de fichier (ILIKE) sur `/documents?q=...`
  - Déjà implémenté dans Phase 2 initiale
...
**Date de fin : 2026-04-27**

Toutes les tâches de la Phase 2 ont été implémentées :
- ✓ A. Notifications par email (A1-A5)
- ✓ B. Authentification LDAP/AD (B1-B5)
- ✓ C. Gestion des rôles (C1-C4)
- ✓ D. Export rapports de conformité (D1-D3)
- ✓ E. API REST (E1-E6)
- ✓ F. Améliorations diverses (F1-F3)

### Nouveaux fichiers créés :
- `app/models/setting.py` — Paramètres globaux
- `app/services/report_service.py` — Envoi automatique des rapports
- `app/templates/admin/roles.html` — Gestion des rôles
- `app/templates/admin/role_form.html` — Formulaire rôle
- `app/templates/admin/parametres.html` — Paramètres globaux
- `app/templates/admin/ldap.html` — Configuration LDAP
